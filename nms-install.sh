...
validate_template_files() {    
    ...
    declare -A required_structure=(
        ...
        ["$script_dir/scripts/"]="restart.sh upgrade.sh"
    )
...
containers_are_running() {
    ...
    for name in "${!arr[@]}"; do
        if [[ "${arr[$name]}" = "true" ]]; then
            check=$(docker ps --filter name=${name} | grep -w ${name})
            if [ -n "${check}" ]; then
...
select_json() {
    while true; do
        # Duplicate file descriptor 1 on 3
        exec 3>&1
        
        local file=$(dialog --stdout --title "Select a JSON file" --fselect $HOME/ 14 48)
        local retval=$?
        ...
        case $retval in
            0)
                # Check if a file was selected and the variable is not empty
                if [[ -n $file ]]; then
                    echo "$file"
                    break
                else
                    # No file selected or Cancel was pressed, return to main menu or handle as needed
                    main_menu
                fi
            ;;
            1)
                # on Cancel, return to main menu
                main_menu
            ;;
            255)
                # on ESC, return to main menu
                main_menu
            ;;
        esac
    done
...
deploy_docker_compose_component() {
    ...
    local file_path="${script_dir}/docker-compose-templates/${file_name}"
...
deploy_config() {
    local nms_dir="${1}"
    local config_name="${2}"
    local file_path="${script_dir}/config-templates/${config_name}_conf_template.yml"
...
install_nms() {
    ...
    # versions .env file
    mv "${script_dir}/versions.env" "${install_dir}/docker-compose/.env" || {
    ...
    bash "${install_dir}/scripts/restart.sh" -a || {
...
redeploy_files() {
    ...
    # Stop all containers
    bash "$dir/scripts/restart.sh" -s -a || {
...
uninstall_nms_option () {
    ...
    echo "Uninstallting NMS"
    echo "This process will stop all NMS containers and delete the NMS directory ($install_dir)"
...
uninstall_nms() {
    ...
    # Ensure that the directory exists and contains scripts/restart.sh   
    if [[ ! -f "$dir/scripts/restart.sh" ]]; then
